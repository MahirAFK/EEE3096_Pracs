...
int main(void)
{
  ...
  while (1)
  {
	...
	sprintf(adcValueStr, "%lu", adcValue); // Convert uint32_t to string
	...
	uint32_t crr_val = ADCtoCCR(adcValue);
...
void EXTI0_1_IRQHandler(void)
{
	// TODO: Add code to switch LED7 delay frequency
	 // Toggle the LED7 delay frequency between 1 Hz and 2 Hz
	  uint32_t currentTime = HAL_GetTick();
	  if (currentTime - lastToggleTime >= 100) {
	    // Toggle frequency if at least 500 ms has passed since the last toggle
